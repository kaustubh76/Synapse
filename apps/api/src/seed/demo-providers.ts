...
// Seed data for testing and demo purposes ONLY
// These providers are NOT real - they are for local development testing
// Set SKIP_DEMO_PROVIDERS=true in .env to disable for production
// ============================================================

import { ProviderRegistry } from '@synapse/core';
import { ethers } from 'ethers';

// Generate deterministic test addresses (NOT real wallets, just valid format)
// These are derived from a test mnemonic and should NEVER hold real funds
function generateTestAddress(seed: string): string {
  const hash = ethers.keccak256(ethers.toUtf8Bytes(`synapse_demo_${seed}`));
  return ethers.getAddress('0x' + hash.slice(26)); // Take last 40 hex chars
}

export function seedDemoProviders(providerRegistry: ProviderRegistry): void {
  console.log('⚠️  Seeding demo providers (set SKIP_DEMO_PROVIDERS=true to disable)...');

  // Weather Bot 1 - High reputation, fast
  const weatherBot1 = providerRegistry.registerProvider({
    name: 'WeatherBot Pro [DEMO]',
    ...
    address: generateTestAddress('weatherbot1')
  });
  // Boost reputation (simulated history)
  for (let i = 0; i < 50; i++) {
  ...
  const cryptoBot1 = providerRegistry.registerProvider({
    ...
    address: generateTestAddress('cryptobot1')
  });
  for (let i = 0; i < 100; i++) {
  ...
  const newsBot1 = providerRegistry.registerProvider({
    ...
    address: generateTestAddress('newsbot1')
  });
  for (let i = 0; i < 30; i++) {
  ...
  const aiBot = providerRegistry.registerProvider({
    ...
    address: generateTestAddress('aibot1')
  });
  for (let i = 0; i < 25; i++) {
  ...
  console.log(`
╔═══════════════════════════════════════════════════════════════╗
║           ⚠️  DEMO PROVIDERS SEEDED (NOT FOR PRODUCTION) ⚠️    ║
╠═══════════════════════════════════════════════════════════════╣
...
╠═══════════════════════════════════════════════════════════════╣
║  Set SKIP_DEMO_PROVIDERS=true in .env to disable for prod    ║
╚═══════════════════════════════════════════════════════════════╝
  `);
}
